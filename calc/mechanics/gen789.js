...
function calculateSMSSSV(gen, attacker, defender, move, field) {
    ...
    var isDarkRevealed = attacker.hasAbility('Mindbreaker');
    var isRingTarget = defender.hasItem('Ring Target') && !defender.hasAbility('Klutz');
    var type1Effectiveness = (0, util_2.getMoveEffectiveness)(gen, move, defender.types[0], isGhostRevealed, field.isGravity, isRingTarget, isDarkRevealed);

    if (attacker.hasAbility('Mindbreaker') && move.type === 'Psychic' &&
        defender.types[0] === 'Dark' && type1Effectiveness === 0) {
        type1Effectiveness = 1;
    }
    var type2Effectiveness = defender.types[1]
        ? (0, util_2.getMoveEffectiveness)(gen, move, defender.types[1], isGhostRevealed, field.isGravity, isRingTarget, isDarkRevealed)
        : 1;
    if (attacker.hasAbility('Mindbreaker') && move.type === 'Psychic' &&
        defender.types[1] === 'Dark') {
        if (type2Effectiveness === 0) {
            type2Effectiveness = 1;
        }
        else if (type2Effectiveness === 1) {
...
function calculateAttackSMSSSV(gen, attacker, defender, move, field, desc, isCritical) {
    ...
    if (attacker.hasAbility('Striker') && move.flags.kick === 1) {
        attack = attack*1.3;
        desc.attackerAbility = attacker.ability;
    }
