...
function calculateDPP(gen, attacker, defender, move, field) {
    ...
    // Mindbreaker: Psychic moves deal neutral damage to Dark types
    if (attacker.hasAbility('Mindbreaker') && move.hasType('Psychic')) {
        if (firstDefenderType === 'Dark' && type1Effectiveness === 0) {
            type1Effectiveness = 1;
        }
    }

        if (secondDefenderType === 'Dark' && type2Effectiveness === 0) {
